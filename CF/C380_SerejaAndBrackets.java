...
public class C380_SerejaAndBrackets {

	public static void main(String[] args) throws IOException {
		Scanner in =new Scanner(System.in);
		StringBuilder sb =new StringBuilder();
		String s = in.nextLine();
		int N = 1;int n = s.length();
		while(N < n)	N <<= 1;
		Trio [] a=new Trio [N+1];
		
		for(int i=1;i<=s.length();i++)
			a[i] = (s.charAt(i-1)== '(' )? new Trio(1,0,0):new Trio(0,1,0);
	...
	static class Trio
	{
		int op;int clos;int ressofar;
		Trio(int a, int b, int c)
		{
			op=a;clos=b;ressofar=c;
	...
	static class SegmentTree{
		...
		void build(int node, int b, int e)
		{
			...
			Stree[node] = new Trio(open, close, res*2+left.ressofar+right.ressofar);
			}
		}
		...
		Trio query(int node, int b, int e, int i, int j)
		{
			...
			return  new Trio(open, close, res*2+left.ressofar+right.ressofar);
