			...
			for(int i=0;i<n;i++)
			{
				for(int j=0;j<m;j++)
				{
					if(grid[i][j] == '*')
						continue;
					int node = conv(i,j);
					int cost = dist2[node] + dist3[node] + dist1[node] - ((grid[i][j] == '#' )? 2:0);//2 because the cost at dist1[node] will conatin dist from p1 and from p2 and from the outside part
