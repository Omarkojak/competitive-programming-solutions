...
public class Uva1172_TheBridgesOfKolsberg {
	...
	static Pair max(Pair p1, Pair p2, Pair p3)
	{
		...
		int min = inf;
		if(p1.sum == max)	min = Math.min(min, p1.num);
		if(p2.sum == max)	min = Math.min(min, p2.num);
		if(p3.sum == max)	min = Math.min(min, p3.num);
	...
	public static void main(String[] args) throws IOException {
		...
		{
			n1 = in.nextInt();
			os1 = new String[n1]; trade1 = new int [n1];
			for(int i=0;i<n1;i++)
			{
				in.next();
				os1[i] = in.next();
			...
			n2 = in.nextInt();
			os2 = new String[n2]; trade2 = new int [n2];
			for(int i=0;i<n2;i++)
			{
				in.next();
				os2[i] = in.next();
