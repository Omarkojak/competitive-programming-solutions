		...
		for (int i = 0; i < 4; i++) {
			if (!biton[i][0]) {
				...
				if(next == id)
					return win[id][msk] = id;
				if (!biton[i][1]) {
					next = solve(id ^ 1, msk | (1 << conv(i, 0) | (1 << conv(i, 1))));
					if(next == id)
						return win[id][msk] = id;
					if (!biton[i][2])
						next = solve(id ^ 1, msk | (1 << conv(i, 0) | (1 << conv(i, 1) | (1 << conv(i, 2)))));
					if(next == id)
						return win[id][msk] = id;
				}
			}
			...
			if (!biton[i][3]) {
				...
				if(next == id)
					return win[id][msk] = id;
				if (!biton[i][2]) {
					next = solve(id ^ 1, msk | (1 << conv(i, 3) | (1 << conv(i, 2))));
					if(next == id)
						return win[id][msk] = id;
					if (!biton[i][1])
						next = solve(id ^ 1, msk | (1 << conv(i, 3) | (1 << conv(i, 2) | (1 << conv(i, 1)))));
					if(next == id)
						return win[id][msk] = id;
				}
			}
		...
		for (int j = 0; j < 4; j++) {
			if (!biton[0][j]) {
				...
				if(next == id)
					return win[id][msk] = id;
				if (!biton[1][j]) {
					next = solve(id ^ 1, msk | (1 << conv(0, j) | (1 << conv(1, j))));
					if(next == id)
						return win[id][msk] = id;
					if (!biton[2][j])
						next = solve(id ^ 1, msk | (1 << conv(0, j) | (1 << conv(1, j) | (1 << conv(2, j)))));
					if(next == id)
						return win[id][msk] = id;
				}
			}
			...
			if (!biton[3][j]) {
				...
				if(next == id)
					return win[id][msk] = id;
				if (!biton[2][j]) {
					next = solve(id ^ 1, msk | (1 << conv(3, j) | (1 << conv(2, j))));
					if(next == id)
						return win[id][msk] = id;
					if (!biton[1][j])
						next = solve(id ^ 1, msk | (1 << conv(3, j) | (1 << conv(2, j) | (1 << conv(1, j)))));
					if(next == id)
						return win[id][msk] = id;
				}
			}
